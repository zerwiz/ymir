...
read `svartalfaheim/<realm>/.env.realm` for the **active realm** — which defaults to
`wayof` and can be set with `BROKK_REALM`. The values are loaded into the process
environment; they are never printed, echoed into a log, or written to a rune.
...
```bash
cd svartalfaheim/wayof/companies/<company>
cp .env.example .env
```
...
```bash
git check-ignore -v svartalfaheim/wayof/.env.realm     # must be ignored
