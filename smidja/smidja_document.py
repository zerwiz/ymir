...
# ///
"""smidja Document — write up the work that was just done, from the diff.

Usage:
    uv run smidja/smidja_document.py "<prompt or path/to/prompt.md>" [--base main] [--config smidja/smidja_smidja_config/smidja.config.yaml] [--smidja-id a1b2c3d4]
...
means here; see smidja_modules/changes.py for how the base commit is resolved on a
...
from smidja_modules import agents, changes, gates, session, utils
from smidja_modules.data_types import (AgentCall, ChangeCapture, DocumentOutput,
                                    PhaseParams)
...
if __name__ == "__main__":
    ...
    sys.exit(main(utils.resolve_prompt(args.prompt), args.base, args.config, args.smidja_id))
