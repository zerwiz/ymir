...
PATTERNS='(AKIA[0-9A-Z]{16}|-----BEGIN [A-Z ]*PRIVATE KEY-----|(ghp|gho|ghs|ghr)_[A-Za-z0-9]{36}|github_pat_[A-Za-z0-9_]{20,}|sk-[A-Za-z0-9]{20,}|xox[baprs]-[A-Za-z0-9-]+|AIza[0-9A-Za-z_-]{35}|CLOUDFLARE_API_TOKEN[[:space:]]*=[[:space:]]*[^[:space:]]|OPENCODE_GO_API_KEY[[:space:]]*=[[:space:]]*[^[:space:]]|GITHUB_TOKEN[[:space:]]*=[[:space:]]*[^[:space:]])'
...
case "${1:-}" in
  ...
  --install)
    ...
    printf '#!/usr/bin/env bash\nexec "%s" "$@"\n' "$SCRIPT_DIR/secret-guard.sh" >"$hook"
    chmod +x "$hook"
...
scan_list() {
  ...
  while IFS= read -r f; do
    ...
    if git -C "$ROOT" show ":$f" 2>/dev/null | grep -Eq "$PATTERNS"; then
