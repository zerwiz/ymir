...
#
# Usage:
#   brokk-lease.sh claim <task> [--actor main|branch]
...
#   brokk-lease.sh release <task> [--actor main|branch]
...
#
# The default actor is $BROKK_SUPERVISION_ACTOR (else main); when --actor is
...
usage() {
  echo "usage: brokk-lease.sh claim|release <task> [--actor main|branch] | release-actor --actor main|branch | check <task> | sweep" >&2
  exit 2
}
