#!/usr/bin/env bash
# brokk-lease-lib.sh - the per-task supervision lease contract (one owner).
#
# WHY. On the Pi supervision branch (docs/pi-supervision-branch.md), two LLM
# actors share one brokk home inside one pi process: MAIN (the Allfather's
...
#     lease-command lock; leases never coordinate across brokk homes.
#   - Actors: exactly "main" and "branch". The current actor is
#     $BROKK_SUPERVISION_ACTOR when set, else "main". The branch's shell gets
#     BROKK_SUPERVISION_ACTOR=branch injected deterministically by the Pi branch
...
#     refuses with exit BROKK_LEASE_REFUSE_EXIT. In a Pi supervision context the
...
BROKK_LEASE_REFUSE_EXIT=6
BROKK_LEASE_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BROKK_LEASE_GUARD_LOCK=

fm_lease_lock_helpers() {
  command -v fm_lock_acquire_wait >/dev/null 2>&1 && return 0
  # brokk-wake-lib.sh is a canonical lint root in its own right and is already
...
# stderr) for an unknown BROKK_SUPERVISION_ACTOR value.
fm_lease_actor() {
  local actor=${BROKK_SUPERVISION_ACTOR:-main}
  case "$actor" in
    main|branch) printf '%s\n' "$actor" ;;
    *)
      echo "error: unknown BROKK_SUPERVISION_ACTOR '$actor' (expected main or branch)" >&2
      return 1
      ;;
  esac
}
...
# fm_lease_read <task>: read the lease into BROKK_LEASE_ACTOR/BROKK_LEASE_PID/
# BROKK_LEASE_EPOCH. Returns 1 when no lease file exists. A malformed lease
...
fm_lease_live() {
  local lock_pid
  case "${PI_CODING_AGENT:-}:${BROKK_SUPERVISION_ACTOR:-}" in
    true:*|*:main|*:branch) ;;
    *) return 1 ;;
  esac
...
# fm_lease_guard <task> <action-label>: refuse (exit BROKK_LEASE_REFUSE_EXIT) when
...
fm_lease_guard() {
  ...
  actor=$(fm_lease_actor) || exit "$BROKK_LEASE_REFUSE_EXIT"
  case "${PI_CODING_AGENT:-}:${BROKK_SUPERVISION_ACTOR:-}" in
    true:*|*:main|*:branch) active=1 ;;
  esac
  [ "$active" = 1 ] || [ -e "$(fm_lease_path "$task")" ] || return 0
  fm_lease_lock_helpers
  lock="$STATE/.brokk-lease-command.lock"
  ...
  if [ "$BROKK_LEASE_GUARD_LOCK" != "$lock" ]; then
    fm_lock_acquire_wait "$lock"
    BROKK_LEASE_GUARD_LOCK=$lock
  fi
...
fm_lease_guard_release() {
  local lock=$BROKK_LEASE_GUARD_LOCK
  [ -n "$lock" ] || return 0
  BROKK_LEASE_GUARD_LOCK=
  fm_lock_release "$lock"
}

# fm_lease_forbid_branch <action-label>: refuse (exit BROKK_LEASE_REFUSE_EXIT)
