#!/usr/bin/env bash
# skuld-branch-outcome.sh - the durable outcome store for the Pi supervision
...
#     "verdict":"routine"|"Allfather","summary":"...","silent":true|false}.
...
#     and truncation, if ever needed, is a Allfather-approved manual act.
...
#     when the session holds the lock (saga-session-start.sh owns the call site).
set -eu
...
usage() {
  echo "usage: skuld-branch-outcome.sh append --task <id> --verdict routine|Allfather --summary <text> [--wake <text>] [--silent true|false] | unread | mark-read --through <seq> | list [--recent <n>] | startup-replay" >&2
  exit 2
}
...
last_seq() {
  ...
  value=$(tail -n 1 "$STORE" 2>/dev/null | jq -er '
    ...
    | select((.summary | type) == "string" and (.verdict == "routine" or .verdict == "Allfather"))
    | .seq
  ') || return 1
  printf '%s\n' "$value"
}
...
case "$CMD" in
  append)
    ...
    [ -n "$TASK" ] || usage
    [ -n "$SUMMARY" ] || usage
    case "$VERDICT" in routine|Allfather) ;; *) usage ;; esac
