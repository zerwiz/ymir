...
for p in d:
    ...
    if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
      while IFS=$'\t' read -r name status; do
        [ -n "$name" ] && rows+=("docker:$name|docker|$name|$status|0|0|0|0")
      done < <(docker ps --format '{{.Names}}\t{{.State}}' 2>/dev/null)
    fi
    ...
    case "$mgr" in
      ...
      docker)
        case "$CMD" in
          ...
          start) docker start "$name" >/dev/null 2>&1 ;;
          restart) docker restart "$name" >/dev/null 2>&1 ;;
        esac && printf 'process[1]{id,action,ok}:\n  "%s","%s","yes"\n' "$id" "$CMD" || { printf 'error: docker %s failed\n' "$CMD" >&2; exit 1; } ;;
  ...
  logs)
    ...
    case "$mgr" in
      ...
      docker) docker logs --tail "$n" "$name" 2>&1 | tail -n "$n" ;;
