...
cleanup() {
  ...
  [ "$LOCK_HELD" = false ] || fm_lock_release "$BROKK_WAKE_QUEUE_LOCK"
  exit "$status"
}
trap cleanup EXIT
trap 'exit 130' INT
...
owner_matches() {
  ...
  exec 8<&-
  [ "$version" = brokk-branch-eligible-owner-v1 ] || return 1
...
case "${1:-}" in
  activate)
    ...
    printf '%s\n%s\n%s\n%s\n' brokk-branch-eligible-owner-v1 "$pid" "$identity" "$generation" > "$TMP" || exit 1
  ...
  deactivate)
    ...
    ;;
  *)
    echo "usage: brokk-wake-grant.sh activate PID GENERATION | publish GENERATION SEQUENCE... | release GENERATION | deactivate PID GENERATION" >&2
    exit 2
    ;;
esac
