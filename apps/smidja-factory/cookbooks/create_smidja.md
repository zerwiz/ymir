...
Compose a new smidja script — a thin, deterministic Python workflow over agents already in the config. Design the chain first, then generate or hand-write it.

## Step 1 — Design the chain

Answer four questions, in order:

1. **What agents, in what order?** Pick from the roster (`smidja/smidja_smidja_config/smidja.config.yaml`). The starter six cover most chains:
...
- `kind="engineer"` → `owner=run.engineer`. Every smidja opens with the engineer request phase — it is the system input record.
...
```bash
uv run .claude/skills/smidja/scripts/make_adw.py --name review_docs --agents scout,builder
```

Writes `smidja/smidja_review_docs.py`: one agent phase per name, chained by `previous=`, starter agents mapped to their output types, unknown agents to `GenericOutput`. It does NOT create config entries or prompt files — do that first (`update_config.md`), or `agents.validate()` will stop the run and tell you what's missing.

## The canonical skeleton

Every `smidja_*.py`, generated or hand-written, is a `uv` single-file script with this shape:
...
# ///
"""smidja Plan Build — plan the request, then implement the plan."""
...
def main(prompt: str, config: str = "smidja/smidja_smidja_config/smidja.config.yaml", smidja_id: str | None = None) -> int:
    ...
    run = session.ensure(cfg, smidja_id)           # 3. pin-or-create the session → the Run object
...
- **Stay thin** — sequencing and acceptance only; real logic goes in `smidja_modules/` (`update_modules.md`).
- **Committing is a code phase, and it needs a fallback.** `PlanOutput`, `BuildOutput`, and `DocumentOutput` each carry a `commit_message` the agent writes **for its own work product** — the spec, the code, the write-up. It defaults to empty, so always `envelope.commit_message or <fallback>`, and commit each product with the message of the agent that made it (`smidja_simple_sdlc.py` commits three times and never crosses them). `git_helper.commit_all(message)` stages everything, commits, and returns the short sha; it raises a clear error when the cwd isn't a git repo or nothing changed, and that raise fails the phase.

## Before you ship it

1. `uv run smidja/smidja_<name>.py "a tiny real request"` — watch it go green end to end.
2. Check the trace: `sqlite3 smidja/smidja_data/smidja.db "select seq,name,kind,owner,status from phases where smidja_id='<id>' order by seq;"`
