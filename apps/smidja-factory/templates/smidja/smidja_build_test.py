...
# ///
"""smidja Build Test — implement, then verify; failures flow back into the builder.

Usage:
    uv run smidja/smidja_build_test.py "<prompt or path/to/prompt.md>" [--config smidja/smidja_smidja_config/smidja.config.yaml] [--smidja-id a1b2c3d4]

Phases: engineer(request) -> builder -> code(test) [-> builder(fix) -> code(test) ... bounded]

Testing is CODE. The suite's command is written down in smidja_modules/quality.py,
...
from smidja_modules import agents, gates, quality, session, utils
