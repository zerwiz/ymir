"""OpenCode coding-agent interface — the smidja can run through opencode CLI.
...
rejects unknown session ids, so a fresh run created with a smidja id
(`smidja-...`) is launched WITHOUT `--session`; the real `ses_...` id from the
