# Smíðja Enhancement Plan

**Date**: September 2026  
**Scope**: Visualizer (frontend) + Smíðja Backend (Python)  
**Purpose**: Comprehensive roadmap for extending the smithy observability and execution capabilities

---

## Executive Summary

The Smíðja is a **Smíðja** — deterministic Python scripts own sequencing/retries/acceptance; coding agents (Pi/opencode) work inside bounded phases; typed JSON envelopes carry context; everything streams into SQLite for the polled visualizer.
...
### Smíðja Backend (`templates/smidja/`)
| Layer | Technology | Key Files |
|-------|------------|-----------|
| Orchestration | Python 3.11+ (uv scripts) | `smidja_*.py` |
| Data Contracts | Pydantic v2 | `smidja_modules/data_types.py` |
| Agent Runtime | `agent_pi.py` (Pi), `agent_opencode.py` (opencode) | |
| Tracing | SQLite WAL + JSONL events | `smidja_modules/tracer.py` |
| Quality Gates | Deterministic subprocess runs | `smidja_modules/quality.py` |
| Permissions | Path-based write enforcement | `smidja_modules/permissions.py` |
| Context Handoff | File-based `context_handoff/` dir | `smidja_modules/agents.py` |
| Sub-agents | Task-tool lane materialization (G2) | `smidja_modules/agents.py` |

**Current Smithies**: `smidja_scout`, `smidja_simple_sdlc`, `smidja_plan_build`, `smidja_build_test`, `smidja_plan_build_test`, `smidja_plan_build_test_quality`, `smidja_build_review`, `smidja_document`, `smidja_orchestrate`, `smidja_recon_iv`, `smidja_prompt`, `smidja_quality`
...
| A.3.2 | **Cross-project memory search** | Query Kaia memory across all smidja repos from one UI | Medium | Medium |
...
| A.4.2 | **Chat-to-smidja trace linking** | Click a launched session in chat → jump to its trace (exists) + back-link | Small | High |
...
| A.5.3 | **Smíðja chain builder** | Visual pipeline editor for creating custom smidja scripts | Large | Medium |
...
### C. Smíðja Backend — Execution & Orchestration

#### C.1 New Smíðja Chains (Templates)
| ID | Enhancement | Description | Effort | Priority |
|----|-------------|-------------|--------|----------|
| C.1.1 | **`smidja_security_audit`** | Scout → threat model → code review → fix → retest → document | Medium | High |
| C.1.2 | **`smidja_perf_optimize`** | Benchmark → profile → optimize → benchmark → regression test | Medium | High |
| C.1.3 | **`smidja_migration`** | Analyze → plan migration → execute → verify → rollback plan | Medium | Medium |
| C.1.3 | **`smidja_dependency_update`** | Scan → plan updates → test → staged rollout → verify | Small | Medium |
| C.1.4 | **`smidja_incident_response`** | Triage → diagnose → fix → verify → postmortem → Kaia memory | Medium | High |
| C.1.5 | **`smidja_feature_flag_rollout`** | Gradual rollout with metric gates + automatic rollback | Medium | Medium |
...
| C.4.2 | **Cross-repo context sharing** | Shared `context_handoff/` across monorepo smithies | Medium | Medium |
...
| C.5.2 | **Cron / scheduled smithies** | `smidja schedule "0 2 * * *" smidja_simple_sdlc "nightly refactor"` | Medium | Medium |
| C.5.3 | **Event-driven smithies** | `process-event-sources` → smidja trigger (GitHub webhook, cron, etc.) | Medium | High |
| C.5.4 | **Smíðja composition** | `smidja_chain: [smidja_plan, smidja_build_test, smidja_document]` | Medium | Medium |
| C.5.5 | **Distributed execution** | Offload agent phases to remote workers (Kubernetes, modal, fly.io) | Large | Low |

---

### D. Smíðja Backend — Developer Experience

#### D.1 Configuration & Onboarding
| ID | Enhancement | Description | Effort | Priority |
|----|-------------|-------------|--------|----------|
| D.1.1 | **`smidja init --interactive`** | Guided setup: language, test runner, agents, models, quality blocks | Small | High |
| D.1.2 | **Config validation CLI** | `smidja doctor` — full preflight (models, tools, prompts, perms) | Small | High |
| D.1.3 | **Config schema docs generator** | Auto-generate markdown from `smidjaConfig` Pydantic model | Small | Medium |
| D.1.4 | **Roster inheritance** | `extends: base-roster` in YAML for shared agent definitions | Small | Medium |

#### D.2 Debugging & Diagnostics
| ID | Enhancement | Description | Effort | Priority |
|----|-------------|-------------|--------|----------|
| D.2.1 | **`smidja replay <session>`** | Re-run a session from any phase with same context (deterministic replay) | Medium | High |
| D.2.2 | **`smidja diagnose <session>`** | Auto-analyze failure: root cause, suggested fix, similar past failures | Medium | High |
| D.2.3 | **Live agent REPL** | `smidja shell <session> <agent>` — interactive prompt in agent's context | Large | Medium |
| D.2.4 | **Phase time-travel** | `smidja phase <session> <phase> --at <timestamp>` — inspect state at point | Medium | Low |

#### D.3 Testing & Validation
| ID | Enhancement | Description | Effort | Priority |
|----|-------------|-------------|--------|----------|
| D.3.1 | **Smíðja contract tests** | Test smidja scripts against known-good fixtures (golden runs) | Medium | High |
| D.3.2 | **Agent prompt regression suite** | Test agent prompts against model versions for drift detection | Medium | Medium |
| D.3.3 | **Chaos testing harness** | Inject faults (network, model errors, OOM) into smidja runs | Large | Low |
...
| ID | Enhancement | Description | Effort | Priority |
|----|-------------|-------------|--------|----------|
| E.1.1 | **Docker/OCI images** | Pre-built images for smidja runner + visualizer + Kaia bridge | Medium | High |
| E.1.2 | **Kubernetes operator** | `SmidjaRun` CRD for cluster-native execution + visualizer ingress | Large | Medium |
| E.1.3 | **GitHub App integration** | Installable app: PR checks, status checks, auto-smidja on labels | Large | High |
...
| ID | Enhancement | Description | Effort | Priority |
|----|-------------|-------------|--------|----------|
| E.2.1 | **Team workspaces** | Isolated smidja configs, visualizer instances, Kaia memory per team | Large | Medium |
| E.2.2 | **RBAC for visualizer** | Viewer/Operator/Admin roles per workspace | Medium | Medium |
| E.2.3 | **Audit log** | Immutable log of all smidja runs, steers, stops, config changes | Medium | High |
...
| D.1.1 `smidja init --interactive` | Backend | Guided setup wizard |
| D.1.2 `smidja doctor` | Backend | Full preflight validation |
...
| C.1.1 Security audit smidja | Backend | New `smidja_security_audit.py` |
| C.1.2 Perf optimize smidja | Backend | New `smidja_perf_optimize.py` |
| C.1.5 Incident response smidja | Backend | New `smidja_incident_response.py` |
...
| C.5.3 Event-driven smithies | Backend | `process-event-sources` integration |
| D.2.1 `smidja replay` | Backend | Deterministic session replay |
| D.2.2 `smidja diagnose` | Backend | Auto failure analysis |
...
| A.5.3 Smíðja chain builder | Visualizer | Visual pipeline editor |
...
| C.1.3 Migration smidja | Backend | `smidja_migration.py` |
| C.1.4 Dependency update smidja | Backend | `smidja_dependency_update.py` |
| C.1.5 Feature flag rollout | Backend | `smidja_feature_flag_rollout.py` |
...
| C.5.2 Cron scheduled smithies | Backend | `smidja schedule` command |
| C.5.4 Smíðja composition | Backend | Chain smidja scripts |
...
| D.3.1 Smíðja contract tests | Backend | Golden run test suite |
| D.3.2 Agent prompt regression | Backend | Model drift detection |
| E.1.2 K8s operator | Infra | `SmidjaRun` CRD |
...
1. **Plugin architecture**: Smíðja modules should be discoverable plugins, not hardcoded imports
2. **Async execution**: `agent_pi.run` is synchronous; move to `asyncio` for parallel phases
3. **Config hot-reload**: Watch `smidja.config.yaml` for changes without restart
4. **Structured logging**: Replace `console.py` Rich output with structured JSON logs + OTel
5. **Migration system**: Versioned DB migrations for `smidja.db` schema changes
...
| Smíðja script authoring time | ~2 hours | <30 min (visual builder) | <15 min |
...
| Smíðja adoption (repos stamped) | ~5 | 25 | 100+ |
...
# Smíðja
uv run smidja/smidja_simple_sdlc.py "add health endpoint"
uv run smidja/smidja_orchestrate.py "refactor auth" --smidja-id a1b2c3d4
smidja doctor             # Preflight check
smidja team list          # Show rosters
smidja mission T1         # Bench missions
...
| Smíðja chains | `templates/smidja/smidja_*.py` |
| Smíðja modules | `templates/smidja/smidja_modules/*.py` |
| Data contracts | `templates/smidja/smidja_modules/data_types.py` |
| Agent prompts | `templates/prompt_engineering/<agent>/system.md`, `user.md` |
| Config schema | `templates/smidja.config.yaml`, `references/config.md` |
...
1. **Allfather reviews and prioritizes** — Select Phase 1 items for immediate sprint
