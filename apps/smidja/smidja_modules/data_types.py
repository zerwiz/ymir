"""Concrete data types for the smidja smidja system.
...
class DocumentOutput(EnvelopeBase):
    ...
    document_path: str = ""         # the doc in the repo, e.g. app_docs/<smidja_id>_<slug>.md
...
class BaseRef(BaseModel):
    ...
    thing it was taken against, so the smidja records that choice instead of
...
class VerifyOutput(EnvelopeBase):
    ...
    the smidja script is the only thing that knows the difference.
    """
...
class AgentConfig(BaseModel):
    ...
    # (see smidja_modules/permissions.py). `tools` cannot express this: `bash` runs
...
class ConfigDefaults(BaseModel):
    ...
    # The smidja's own code is the default: an agent must not be able to edit
    # the machinery that decides whether its work passed.
    protected_files: list[str] = Field(default_factory=lambda: [
        "smidja/smidja_modules/", "smidja/smidja_smidja_config/", "smidja/smidja_*.py",
    ])
    data_dir: str = "smidja/smidja_data"


class ObservabilityConfig(BaseModel):
    db: str = "smidja/smidja_data/smidja.db"
    poll_ms: int = 500


class smidjaConfig(BaseModel):
...
class EventRecord(BaseModel):
    """One traced event, always logged against smidja_id + phase."""
