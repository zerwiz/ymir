...
all best-effort):
    ...
    SMIDJA_STALL_WATCH=1          on by default; 0 disables
    SMIDJA_STALL_SECS=180         no-new-bytes threshold before recovery
    SMIDJA_STALL_GPU_PCT=90       GPU util at/above which a stall counts
    SMIDJA_STALL_POLL=5           watchdog sampling interval
"""
...
_ENABLED = os.environ.get("SMIDJA_STALL_WATCH", "1") == "1"
_STALL_S = int(os.environ.get("SMIDJA_STALL_SECS", "180"))
_GPU_PCT = int(os.environ.get("SMIDJA_STALL_GPU_PCT", "90"))
_POLL_S = float(os.environ.get("SMIDJA_STALL_POLL", "5"))


class StallWatch:
    """Watch one phase's raw stream; unload→reload + kill on a GPU-pegged stall."""

    def __init__(self, smidja_id: str, phase_id: str, agent: str,
                 model_key: str, raw_path: str, tracer,
                 poll_s: float = _POLL_S, stall_s: int = _STALL_S,
                 gpu_pct: int = _GPU_PCT) -> None:
        self.smidja_id = smidja_id
    ...
    def _trace(self, type_: str, payload: dict) -> None:
        try:
            ...
            self.tracer.event(EventRecord(
                smidja_id=self.smidja_id, phase_id=self.phase_id,
...
def watch_for(request, run, phase, agent) -> StallWatch | None:
    ...
    if not getattr(request, "model", "").startswith("lmstudio/"):
        return None
    return StallWatch(
        smidja_id=run.smidja_id, phase_id=phase.phase_id, agent=agent.name,
