...
Verified for crew and scout work on tmux on 2026-08-11 and Herdr on 2026-08-12, and for Eindri-home and primary work on 2026-08-13, with Cursor Agent CLI 2026.08.11-e8db854.
...
| Fact | Value |
|---|---|
| Binary | `fm_cursor_resolve_binary` in `../../../bin/brokk-cursor-lib.sh` resolves stable launcher `cursor-agent` or legacy `agent`, never `cursor`; both symlink into `~/.local/share/cursor-agent/versions/<version>/cursor-agent`, whose target auto-update replaces. |
...
| Busy state | `../../../bin/brokk-busy-lib.sh` folds the per-conversation transcript as `cursor-transcript`: `role:user` opens and typed `turn_ended` closes success or abort, covering manual interrupt; nothing is armed or seeded, and this backend-agnostic source was identical on tmux and Herdr. |
| Exit command | `/exit`. |
...
| Skill invocation | `/<skill>`, for example `/no-mistakes`; Cursor discovers Brokk's user skills. |
...
Bare `node` matches nothing; `../../../bin/brokk-cursor-lib.sh` proves identity from Cursor's name or install tree in path or argv zero.
...
`../../../bin/brokk-tmux-lib.sh` lets the bottom-most shape win only after structural Cursor proof.
...
`../../../bin/brokk-supervise-daemon.sh` can therefore require affirmatively empty before away-mode delivery without a Cursor-only branch.
...
Reverse-video placeholder remnants and Herdr half-block edges belong to `../../../bin/brokk-composer-lib.sh`; without the edges a bare composer swallows the footer and idle reads pending.
`../../../docs/verification/runtime-backends.md` owns captures.
Refresh with `BROKK_HARNESS_LIVENESS_DRIFT=1 ../../../bin/brokk-test-run.sh ../../../tests/brokk-harness-liveness-drift-live-e2e.test.sh`.

## Worktree boundary

Brokk enters its acquired worktree and passes the same absolute path through `--workspace`.
...
Example: `../../../bin/einherjar-spawn.sh <task-id> <project> --scout --harness cursor --model cursor-grok-4.5-high`.

## Primary integration

Primary supervision is the stop-hook park in `../../../docs/supervision-protocols/cursor.md` through tracked `.cursor/hooks.json`; primary and Eindri-home launches require `--trust` or hooks do not load.
...
`stop` cannot block because exit 2 is a silent no-op, so `../../../bin/brokk-turnend-guard-cursor.sh` parks on supervision and returns one bounded `followup_message`.
