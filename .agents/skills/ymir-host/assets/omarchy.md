...
The launcher entries alone are not a speed start: reaching Hlidskjalf meant
opening the app menu. Two keys now raise them, generated by
`bin/desktop-place.sh apply` into `~/.config/hypr/ymir-launchers.lua` and
required from `hyprland.lua` like the placement rules:

| Key | App |
|---|---|
| **SUPER + Y** | Ymir Hlidskjalf |
| **SUPER + M** | Ymir Smiðja |

They live in their own file on purpose: `editor-place.sh` regenerates
`ymir-desktops.lua` and preserves only window rules, so binds placed there would
be dropped. Both keys were verified free before use (`omarchy menu keybindings
--print`), and the behaviour is a plain Omarchy `o.bind(label, command)`.
