...
- `bin/syn-watch-arm.sh --restart` verifies a live lock owner, writes `state/.supervision-armed`, touches `state/.watch.heartbeat` each cycle, polls `BROKK_WATCH_POLL_SECONDS` (default 5), and exits on a `signal:`/`stale:`/`check:`/`heartbeat:` line. The harness extension owns continuity and re-arms.
