...
# ///
"""factory Recon IV — long-running recon with interval checkpoints & context resets.
...
frees SPACE, it never rescues RECALL. This factory bounds the work instead:
...
Usage:
    uv run factory/factory_recon_iv.py "<objective>" \
...
factory/factory_data/prompt_engineering/scout/system.md + the interval note below), so
...
Run with one of the recon-interval rosters:
    factory run --roster recon-interval-9b  "<objective>"
    factory run --roster recon-interval-35b "<objective>"
    factory run --roster recon-interval-orch "<objective>"   # Kaia-tier T2 synthesis
"""
...
from factory_modules.data_types import (AgentCall, DocumentOutput, EventRecord,
                                    PhaseParams, ScoutOutput)

REQUIRED_AGENTS = ["recon_orchestrator", "scout", "documenter"]
DEFAULT_MAX_INTERVALS = int(os.environ.get("FACTORY_RECON_INTERVALS", "5"))
DEFAULT_INTERVAL_TOOLS = int(os.environ.get("FACTORY_RECON_INTERVAL_TOOLS", "25"))
...
DOCUMENT_PROMPT = (
    ...
    "with a descriptive slug (e.g. docs/recon_{factory_id}_<area>.md) and list "
...
def tool_call_count(run, phase_id: str) -> int:
    ...
    try:
        ...
        n = con.execute("SELECT COUNT(*) FROM events WHERE factory_id=? AND phase_id=? "
                        "AND type='tool_call'", (run.factory_id, phase_id)).fetchone()[0]
        con.close()
        return int(n)
...
def main(ask: str, config: str = "factory/factory_factory_config/factory.config.yaml",
         factory_id: str | None = None, max_intervals: int = DEFAULT_MAX_INTERVALS,
    ...
    for i in range(1, max_intervals + 1):
        ...
        run.tracer.event(EventRecord(
            factory_id=run.factory_id, phase_id=run.phases[-1].phase_id, type="log",
            name="recon_interval",
...
if __name__ == "__main__":
    ...
    sys.exit(main(args.ask, args.config, args.factory_id,
