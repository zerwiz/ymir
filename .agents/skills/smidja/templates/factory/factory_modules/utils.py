...
def operator_env() -> dict[str, str]:
    ...
    os.environ gets almost all the way there — but factory launch under `uv run`,
    which prepends its ephemeral venv's bin to PATH and sets VIRTUAL_ENV. That
    venv holds the factory's OWN dependencies (pydantic, pyyaml), not the
    ...
    same `python3` the engineer gets in their terminal. The factory's own imports
