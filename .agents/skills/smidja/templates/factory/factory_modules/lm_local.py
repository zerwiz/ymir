...
Why: the factory runs one coding agent per phase; each pi invocation asks LM
...
Controlled by:
    FACTORY_LM_SERVER       default http://localhost:1234
    FACTORY_UNLOAD_MODELS   1 (default) unload non-targets when needed; 0 = disable
...
SERVER = os.environ.get("FACTORY_LM_SERVER", "http://localhost:1234")
...
def make_room_for(model_key: str) -> None:
    ...
    """
    if os.environ.get("FACTORY_UNLOAD_MODELS", "1") == "0":
        return
    try:
        data = _get("/api/v1/models")
