...
unenforceable on its own:
  ...
  * `bash` runs anything. A builder handed bash to run a test suite can also
    run `git checkout factory/` — which is not hypothetical: one did, discarding
...
Two keys drive it, both in factory.config.yaml:
...
def _glob(pattern: str) -> re.Pattern:
    ...
    `factory/factory_*.py` would match `factory/factory_data/sessions/x/y.py` as well as the
    factory scripts it means. `**` is the way to say "cross directories".
    """
    out, i = [], 0
    while i < len(pattern):
        char = pattern[i]
...
def enforce(run, phase, agent: AgentConfig, before: dict[str, str]) -> list[str]:
    ...
    # A path that was ALREADY dirty when the agent started and is still dirty
    # now ("left as-is") is the operator's own uncommitted work, not an
    # agent change. Failing a run over it (a) blames the agent for work it
    # never did and (b) punishes every run that starts on a dirty tree. Only
    # agent-attributable changes are breaches: files the agent CREATED (not
    # in `before`) or files it DELETED/reverted (in `before`, gone in `after`).
    # Additive — a clean baseline behaves exactly as before.
    agent_changes = [p for p in touched
                     if p not in before or p not in after]
    breaches = [p for p in agent_changes if not permitted(p, agent, run.cfg)]
    if not breaches:
        return touched
