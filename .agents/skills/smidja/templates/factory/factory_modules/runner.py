"""The Run object: config + factory_id + agent_map + tracer + console, bound once.
...
class PhaseHandle:
    ...
    def log(self, **payload) -> None:
        self.run.tracer.event(EventRecord(factory_id=self.run.factory_id,
        ...
        if self.phase.params.kind == "engineer" and "input" in payload:
            self.run.tracer.session_request(self.run.factory_id, str(payload["input"]))
...
class Run:
    def __init__(self, cfg, factory_id: str, tracer, engineer: str,
                 repo_root: str | None = None,
                 data_base: str | Path | None = None):
        self.cfg = cfg
        self.factory_id = factory_id
        self.tracer = tracer
        self.console = Console(tracer, factory_id)
        self.engineer = engineer
        self.phases: list[Phase] = []
        self._subagent_counter = 0   # unique ids for orchestrator task-dispatch lanes
        # Lanes opened at tool_execution_start (G2) keyed by (phase_id, call_id)
        # so the terminal materialization closes the SAME lane instead of
        # opening a duplicate. Additive — absent lanes fall back to old behavior.
        self._open_subagent_lanes: dict[str, dict[str, str]] = {}
        self.tokens = 0
        self.cost = 0.0
        self._seq = tracer.max_phase_seq(factory_id)   # a joined run continues the sequence
        # Project-aware anchor: session.ensure passes the project repo root +
        # its factory/factory_data base (agents work in the project, sessions + db
        # live there too). Falls back to the config-relative defaults.
        self.repo_root = Path(repo_root) if repo_root \
            else git_helper.repo_root()             # where every agent is spawned to work
        base = Path(data_base) if data_base \
            else self.repo_root / cfg.defaults.data_dir
        self.session_dir = ensure_dir(base / "sessions" / factory_id)
    ...
    # ── agent map (factory_id -> per-agent coding-agent session ids) ────────────
    ...
    def add_usage(self, tokens: int, cost: float) -> None:
        self.tokens += tokens
        self.cost += cost
        self.tracer.session_add_usage(self.factory_id, tokens, cost)

    # ── the phase primitive ─────────────────────────────────────────────────
    @contextmanager
    def phase(self, params: PhaseParams):
        self._seq += 1
        phase = Phase(phase_id=f"{self.factory_id}_{self._seq:02d}_{params.name}",
                      factory_id=self.factory_id, seq=self._seq, params=params,
                      status="running", started_at=now_iso())
        self.phases.append(phase)
        self.tracer.phase_upsert(phase)
        self.tracer.event(EventRecord(factory_id=self.factory_id, phase_id=phase.phase_id,
        ...
        try:
            ...
            self.tracer.event(EventRecord(factory_id=self.factory_id, phase_id=phase.phase_id,
                                          ...
                                          payload={"status": "fail"}))
            self.tracer.phase_upsert(phase)
            self.tracer.session_finish(self.factory_id, ok=False)
    ...
    def finish(self, accepted: bool = True, reason: str = "") -> int:
        ...
        Two criteria, not one. Every phase must have passed, AND the factory's own
        ...
        self.tracer.session_finish(self.factory_id, ok=ok)
