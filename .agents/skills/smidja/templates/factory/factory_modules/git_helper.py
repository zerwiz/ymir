"""Low-level git operations for code phases. All low-level logic lives in factory_modules."""
...
def repo_root() -> Path:
    ...
    The git toplevel when there is one, else the process cwd (factory run fine in a
    non-git dir; only a commit phase requires a repo). Always absolute, so it is
    safe to hand to a subprocess regardless of where the factory was launched from.
    """
    if is_repo():
...
def commit_all(message: str) -> str:
    ...
    if not is_repo():
        raise RuntimeError(
            ...
            "repo root (and make a first commit) before running an factory that commits.")
    _git("add", "-A")
...
def merge_base(ref: str, other: str = "HEAD") -> str:
    ...
    working tree. One command covers both cases, so no factory has to branch on it.
    """
