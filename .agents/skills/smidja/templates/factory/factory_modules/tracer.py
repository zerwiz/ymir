...
Files are the raw record; factory.db is the queryable mirror the UI polls.
No push transport — the flow is always: agents -> sqlite -> web ui.
WAL mode so the UI can read while factory processes write.
"""
...
CREATE TABLE IF NOT EXISTS sessions (
  ...
  factory_name      TEXT,                -- factory script(s) run, e.g. "factory_plan + factory_build_test"
  request       TEXT,
  status        TEXT,
  engineer      TEXT,
...
class Tracer:
    ...
    def event(self, record: EventRecord) -> str:
        ...
        self.conn.execute(
            "INSERT INTO events (event_id, factory_id, phase_id, parent_id, type, name,"
            " payload_json, tokens, started_at, ended_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
            (event_id, record.factory_id, record.phase_id, record.parent_id, record.type,
    ...
    # ── sessions ────────────────────────────────────────────────────────────
    def session_start(self, factory_id: str, engineer: str, factory_name: str | None = None) -> None:
        self.conn.execute(
            "INSERT INTO sessions (factory_id, status, engineer, started_at) VALUES (?,?,?,?) "
            "ON CONFLICT(factory_id) DO UPDATE SET status='running'",
            (factory_id, "running", engineer, now_iso()),
        )
        if not factory_name:
            return
        # A joined session chains factory — record each distinct one, in run order.
        row = self.conn.execute("SELECT factory_name FROM sessions WHERE factory_id=?",
                                (factory_id,)).fetchone()
        names = row[0].split(" + ") if row and row[0] else []
        if factory_name not in names:
            names.append(factory_name)
            self.conn.execute("UPDATE sessions SET factory_name=? WHERE factory_id=?",
                              (" + ".join(names), factory_id))

    def session_request(self, factory_id: str, request: str) -> None:
        self.conn.execute("UPDATE sessions SET request=? WHERE factory_id=?",
                          (request[:500], factory_id))

    def session_finish(self, factory_id: str, ok: bool) -> None:
        self.conn.execute(
            "UPDATE sessions SET status=?, ended_at=? WHERE factory_id=?",
            ("success" if ok else "fail", now_iso(), factory_id),
        )
        self.processes_end_all(factory_id)   # nothing of this run is alive any more

    def session_add_usage(self, factory_id: str, tokens: int, cost: float) -> None:
        self.conn.execute(
            "UPDATE sessions SET total_tokens=total_tokens+?, total_cost=total_cost+? WHERE factory_id=?",
            (tokens, cost, factory_id),
        )

    # ── processes (factory_id → pid, so a hung run can be found and killed) ─────
    def process_start(self, factory_id: str, kind: str, name: str, pid: int,
                      command: str) -> None:
        """Record a live process for this run.

        A coding agent that hangs produces no events at all, which is exactly
        when you need its pid — and `ps` cannot tell you which factory_id it
        belongs to. Writing it here makes the trace the answer to "what is this
        run running, and how do I stop it".
        """
        self.conn.execute(
            "INSERT INTO processes (factory_id, kind, name, pid, command, started_at)"
            " VALUES (?,?,?,?,?,?)",
            (factory_id, kind, name, pid, command[:500], now_iso()),
        )

    def process_end(self, factory_id: str, pid: int) -> None:
        """Mark the newest live row for this pid as finished."""
        self.conn.execute(
            "UPDATE processes SET ended_at=? WHERE id = ("
            "  SELECT id FROM processes WHERE factory_id=? AND pid=? AND ended_at IS NULL"
            "  ORDER BY id DESC LIMIT 1)",
            (now_iso(), factory_id, pid),
        )

    def processes_end_all(self, factory_id: str) -> None:
        """Close out every live row for a run — called when the session ends."""
        self.conn.execute(
            "UPDATE processes SET ended_at=? WHERE factory_id=? AND ended_at IS NULL",
            (now_iso(), factory_id),
        )

    # ── phases ──────────────────────────────────────────────────────────────
    def max_phase_seq(self, factory_id: str) -> int:
        ...
        """
        row = self.conn.execute("SELECT MAX(seq) FROM phases WHERE factory_id = ?",
                                (factory_id,)).fetchone()
    ...
    def phase_upsert(self, phase: Phase) -> None:
        p = phase.params
        self.conn.execute(
            "INSERT INTO phases (phase_id, factory_id, seq, name, kind, owner, description,"
            ...
            (phase.phase_id, phase.factory_id, phase.seq, p.name, p.kind, p.owner,
    ...
    def envelope_row(self, phase: Phase, agent: str, output_type: str,
        ...
        self.conn.execute(
            "INSERT INTO envelopes (envelope_id, factory_id, phase_id, agent, output_type,"
            " payload_json, valid, attempt, created_at) VALUES (?,?,?,?,?,?,?,?,?)",
            (f"env_{new_id(12)}", phase.factory_id, phase.phase_id, agent, output_type,
    ...
    def gate_row(self, phase: Phase, gate: str, report: GateReport, attempt: int) -> None:
        ...
        self.conn.execute(
            "INSERT INTO gate_results (factory_id, phase_id, attempt, gate, passed,"
            " violations_json, checks_json, created_at) VALUES (?,?,?,?,?,?,?,?)",
            (phase.factory_id, phase.phase_id, attempt, gate, int(report.passed),
             json.dumps(report.violations),
             json.dumps([c.model_dump() for c in report.checks]), now_iso()),
        )

    def agent_session_row(self, factory_id: str, agent: AgentConfig, session_id: str,
        ...
        ts = now_iso()
        self.conn.execute(
            "INSERT INTO agent_sessions (factory_id, agent, coding_agent, model, color,"
            " session_id, context_tokens, context_window, created_at, last_used_at)"
            " VALUES (?,?,?,?,?,?,?,?,?,?)"
            " ON CONFLICT(factory_id, agent) DO UPDATE SET model=excluded.model,"
            " color=excluded.color, session_id=excluded.session_id,"
            " context_tokens=excluded.context_tokens,"
            " context_window=excluded.context_window,"
            " last_used_at=excluded.last_used_at",
            (factory_id, agent.name, agent.coding_agent, agent.model, agent.color,
