"""Concrete data types for the factory factory system.
...
class DocumentOutput(EnvelopeBase):
    ...
    document_path: str = ""         # the doc in the repo, e.g. app_docs/<factory_id>_<slug>.md
...
class BaseRef(BaseModel):
    ...
    thing it was taken against, so the factory records that choice instead of
...
class VerifyOutput(EnvelopeBase):
    ...
    the factory script is the only thing that knows the difference.
    """
...
class AgentConfig(BaseModel):
    ...
    # (see factory_modules/permissions.py). `tools` cannot express this: `bash` runs
...
class ConfigDefaults(BaseModel):
    ...
    protected_files: list[str] = Field(default_factory=lambda: [
        "factory/factory_modules/", "factory/factory_factory_config/", "factory/factory_*.py",
    ])
    data_dir: str = "factory/factory_data"


class ObservabilityConfig(BaseModel):
    db: str = "factory/factory_data/factory.db"
    poll_ms: int = 500
...
class EventRecord(BaseModel):
    """One traced event, always logged against factory_id + phase."""
