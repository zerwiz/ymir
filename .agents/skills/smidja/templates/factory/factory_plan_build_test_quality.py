...
# ///
"""factory Plan Build Test Quality — full agent chain plus deterministic quality.

Usage:
    uv run factory/factory_plan_build_test_quality.py "<prompt or path/to/prompt.md>" [--config factory/factory_factory_config/factory.config.yaml] [--factory-id a1b2c3d4]
...
from factory_modules import agents, gates, git_helper, quality, session, utils
...
def main(prompt: str, config: str = "factory/factory_factory_config/factory.config.yaml", factory_id: str | None = None) -> int:
    ...
    if verified:
        with run.phase(PhaseParams(name="commit", kind="code", owner="git",
            ...
            message = previous.commit_message or f"factory({run.factory_id}): {previous.summary}"
