...
# ///
"""factory Simple SDLC — plan, build, test, review, document, committing as it goes.

Usage:
    uv run factory/factory_simple_sdlc.py "<prompt or path/to/prompt.md>" [--config factory/factory_factory_config/factory.config.yaml] [--factory-id a1b2c3d4]
...
from factory_modules import agents, changes, gates, git_helper, quality, session, utils
from factory_modules.data_types import (AgentCall, BuildOutput, ChangeCapture,
...
def main(prompt: str, config: str = "factory/factory_factory_config/factory.config.yaml", factory_id: str | None = None) -> int:
    ...
    def commit(ph, envelope) -> None:
        ...
        message = envelope.commit_message or f"factory({run.factory_id}): {envelope.summary}"
        ph.log(sha=git_helper.commit_all(message), message=message)

    def record(ph, result) -> None:
        """Log a deterministic block's verdict — the same shape every factory uses."""
