...
# ///
"""factory Plan Build Test — the full starter chain.

Usage:
    uv run factory/factory_plan_build_test.py "<prompt or path/to/prompt.md>" [--config factory/factory_factory_config/factory.config.yaml] [--factory-id a1b2c3d4]

Phases: engineer(request) -> planner -> builder -> code(test) [-> builder(fix) -> code(test) ... bounded] -> git(commit)

Testing is CODE: the suite's command lives in factory_modules/quality.py, so no
