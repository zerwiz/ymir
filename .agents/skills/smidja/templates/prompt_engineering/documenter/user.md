...
3. Copy that file into the repo under `app_docs/`:
   - **List `app_docs/` before you pick the name.** A session that documents more than once reuses its `<factory_id>`, so the obvious name may already be taken.
   - Base name: `app_docs/<factory_id>_<slug>.md`, where `<factory_id>` is the session directory name inside `context_handoff_dir` (`.../sessions/<factory_id>/context_handoff`) and `<slug>` is two to four kebab-case words naming the work.
   - If a file with that name already exists, use `app_docs/<factory_id>_<slug>_v2.md`, then `_v3`, and so on until the name is free. **Never overwrite an existing write-up** — it describes a change that already shipped.
   - **Copy it, do not retype it.** One bash call does the whole step:
     `mkdir -p app_docs && cp "<context_handoff_dir>/document.md" "app_docs/<factory_id>_<slug>.md"`
...
```json
{
  ...
  "document_path": "app_docs/<factory_id>_<slug>.md",
  "documented_files": ["src/server.ts"],
  "artifacts": ["<context_handoff_dir>/document.md", "app_docs/<factory_id>_<slug>.md"],
