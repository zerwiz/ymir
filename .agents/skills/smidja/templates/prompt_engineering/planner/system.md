...
- List `specs/` before naming that copy and pick a name nothing else holds. Two plans in one session share an `factory_id`, and an overwritten spec is a lost record.
