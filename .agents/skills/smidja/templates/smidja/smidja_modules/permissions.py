...
unenforceable on its own:
  ...
  * `bash` runs anything. A builder handed bash to run a test suite can also
    run `git checkout smidja/` — which is not hypothetical: one did, discarding
...
Two keys drive it, both in smidja.config.yaml:
...
from .data_types import AgentConfig, smidjaConfig
...
def _glob(pattern: str) -> re.Pattern:
    ...
    `smidja/smidja_*.py` would match `smidja/smidja_data/sessions/x/y.py` as well as the
    smidja scripts it means. `**` is the way to say "cross directories".
    """
    out, i = [], 0
    while i < len(pattern):
        char = pattern[i]
...
def always_writable(cfg: smidjaConfig) -> list[str]:
...
def permitted(path: str, agent: AgentConfig, cfg: smidjaConfig) -> bool:
