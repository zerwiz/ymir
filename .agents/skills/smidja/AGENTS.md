---
name: smidja
description: Smíðja — the smithy (the software factory). Deploy and operate repeatable agents+code workflows in any codebase. Use when installing the workshop, creating/running/updating a factory, managing the agent roster in factory.config.yaml, or observing running agent workflows. For LAUNCHING runs use the sibling skills factory-launcher / start-the-factory. Keywords - smidja, smithy, software factory, AI developer workflow, agent pipeline, install factory.
argument-hint: "[install | create factory | run factory | update config | ...]"
---

# Smíðja — the Smithy (the software factory)

> **Norse name:** **Smíðja** (the smithy). Its orchestrator seat is **Völundr**,
> the master smith — the factory's Kaia. Formerly "command-factory" (provenance).
> Registry: `.agents/assets/agents/naming.md`, `docs/lore.md` §XI.

Reusable combination of **agents plus code**: deterministic Python factory scripts own sequencing, retries, and acceptance; coding agents (Pi in v1) work inside bounded phases; typed JSON envelopes carry context between them; everything streams into SQLite for the polled visualizer. Agent proposes, code disposes.
...
- **`factory-launcher`** — the one-command launcher (`scripts/factory`): run any
  chain/mode/service, watch, audit, stop, learn, missions.
- **`start-the-factory`** — exactly how to start agents for work: teams
  (rosters), local/online models, chains, orchestrator, gotchas.
- **`factory-instructions`** — turn a vague ask into a runnable request file.
...
- **Default model:** `lmstudio/qwen3.5-9b` (from `factory.config.yaml`:
  `${FACTORY_LOCAL_MODEL:-lmstudio/qwen3.5-9b}`). Full model surface: local
  ...
  `factory/factory_factory_config/roster.yaml` (the `tiers:` block).
- **Rosters/teams:** `factory/factory_factory_config/roster.yaml` — the ONE file (stacks,
  roles, tools, writes + the `tiers:` model catalog). `factory rosters` lists/validates.
  - **Scaffold commands** (safe writes to that one file): `factory team list|show|
    new|smoke`, `factory agent new`, `factory model set|pin`, and `factory doctor` (the
  ...
  - `roster.yaml` = the named teams (stacks) — pick one with `--roster <name>` /
    `FACTORY_ROSTER=<name>`; each stack lists its agents and per-agent model/tools.
    `tiers:` = the model catalog — every role × backend tier (local,
    local-agile, local-max, local-ui, cloud, cloud-free, cloud-reasoning,
    hybrid-plan-local, hybrid-build-local, review) + `--model`/`FACTORY_*_MODEL`
    override surface.
- **`justfile` (repo root):** the starter recipes — `just scout/sdlc/
  ...
  watch and open the UI. It calls `factory-resolve-config`, honoring
  `FACTORY_CONFIG` → `FACTORY_ROSTER` → `FACTORY_MODEL_TIER` → default `factory.config.yaml`.
- **Visualizer ships with this skill:** `apps/visualizer/` (Vue + vite server).
  Start with `just ui` / `scripts/factory ui` → UI `http://localhost:4601`, API
...
- **Bench missions:** `tests/local-models/missions/` (T1 recon, T2 build, T3
  context probe) via `factory mission T1|T2 [--config <roster>]`; results in
  `tests/local-models/RESULTS.md`.
- **Deep-reference docs:** `docs/command docs/` — `software-factory.md`
  (overview), `software-factory-run.md` (how to run), `software-factory-pi.md` /
  `software-factory-opencode.md` (coding-agent setups), `software-factory-visualizer.md`
  (trace UI), `FactoryAgentsAndModels.md` (roster + model backends source of
...
1. Read [cookbooks/factory_overview.md](cookbooks/factory_overview.md) — the system map.
2. `ls factory/factory_*.py` and read each file's `Phases:` docstring line.
3. Print the factory as a table — name, the chain, one line on when to reach for it — and **wait for the engineer's request.**

```
| factory | Chain | Use when |
|---|---|---|
| factory_scout | engineer → scout | read-only recon; nothing changes |
| factory_simple_sdlc | plan → build → test → review → document, 3 commits | the work is real and its shape is not obvious |
```

**Nothing else.** No trace-db queries, no reading the config or the factory scripts' bodies, no repo inventory, no last-runs summary, no diagnosing an old failure, no "current state" dashboard. None of it was asked for, and it is not free:
...
Two exceptions, both narrow: if the engineer's first message already contains a request, skip the waiting and route it; and if the factory is plainly not installed (no `factory/`, no config), say that in one line instead of the table.

## Orchestrator rules

You run the system, observe the system, and help the user interact with it. **You do no factory work yourself:**

- Never implement, plan, or test in an agent's place — launch the factory and watch it.
- Never edit files inside `factory/factory_data/sessions/` — that is the run record.
- Observe by querying `factory/factory_data/factory.db` (WAL — reads never block writers) **when observing is the task**. This is a capability, not a startup step: query it to follow a run you launched or one the engineer asked about, never to volunteer a status report nobody requested.
...
| Request | Cookbook |
|---|---|
| `/factory install`, set up the factory in this repo | [cookbooks/install.md](cookbooks/install.md) |
| create a new factory / workflow | [cookbooks/create_adw.md](cookbooks/create_adw.md) |
| modify an existing factory chain | [cookbooks/update_adw.md](cookbooks/update_adw.md) |
...
| extend factory_modules with new low-level logic | [cookbooks/update_modules.md](cookbooks/update_modules.md) |
| run / monitor an factory | [cookbooks/how_to_prompt_for_the_eng.md](cookbooks/how_to_prompt_for_the_eng.md) **first**, then [cookbooks/run_adw.md](cookbooks/run_adw.md) |
| turn a request into an factory prompt | [cookbooks/how_to_prompt_for_the_eng.md](cookbooks/how_to_prompt_for_the_eng.md) |
...
1. **Validate before running** — every factory declares `REQUIRED_AGENTS` and calls `agents.validate()` first; a missing/misnamed agent fails before anything spawns.
2. **Typed outputs only** — every agent call pairs with a concrete `EnvelopeBase` subclass in `factory_modules/data_types.py`; parse failures re-prompt the same session (context intact), never restart.
...
6. **factory scripts stay thin** — all low-level logic lives in `factory_modules/`.
...
8. **A known command is code, not an agent** — if you can write the invocation down (`bun test`, `ruff check`), it belongs in a `kind="code"` phase via `factory_modules/quality.py`. Agents are for the parts that need reading and deciding; failures come back to the builder as an envelope either way.
9. **`tools:` is a capability list, `writes:` is the boundary** — `bash` runs anything (including `git checkout`) and `write` reaches any path, so a tool list can never make "this agent changes nothing" true. `writes:` per agent and `protected_files` in defaults are enforced in `factory_modules/permissions.py` after every agent call: unauthorized changes are rolled back and the phase dies. The session runtime under `data_dir` is always writable — a read-only agent is read-only with respect to the REPO, never mute.
10. **Every factory ends in `run.finish()`** — phases passing is not the same as the run being accepted. A test phase that ran a red suite succeeded at its job. Pass `accepted=` so the exit code, the session status, and the banner are decided together and cannot disagree.
...
- **Orchestration:** `factory_orchestrate` — Kaia (orchestrator) dispatches real
...
- **Memory:** Kaia's engram under `factory/factory_data/kaia.engram` (UI `#/memory`,
...
ships in `apps/visualizer/` (see Current runtime above) — `just ui` / `factory ui`
