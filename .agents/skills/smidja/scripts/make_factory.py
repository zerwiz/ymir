...
# dependencies = []
# ///
"""make_adw — generate a new one-shot factory script from agents in the config.
...
concrete type in factory_modules/data_types.py and swap it in).
"""
...
# ///
"""factory {title} — generated by make_adw.

Usage:
    uv run factory/factory_{name}.py "<prompt or path/to/prompt.md>" [--config factory/factory_factory_config/factory.config.yaml] [--factory-id a1b2c3d4]
...
from factory_modules.data_types import AgentCall, PhaseParams, {imports}
...
def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--name", required=True, help="snake_case factory name")
    ...
    dest = Path.cwd() / "factory" / f"factory_{args.name}.py"
    ...
    print(f"run: uv run factory/factory_{args.name}.py \"your request\"")
    return 0
