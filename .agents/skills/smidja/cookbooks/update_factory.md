...
Modify an existing factory chain — add phases, add gates, add a bounded fix loop.
...
A code phase does its work in the block body and logs what it did. The commit phase that closes `factory_plan_build.py` and `factory_plan_build_test.py` is the pattern:
...
Gate claims, not guesses: declared artifacts exist and are non-empty, declared JSON parses, declared changes appear in the diff, declared test commands pass. Never hardcode counts — express quantity as a property of the declared list ("at least one artifact", "ALL declared paths valid"). Plan quality and code taste are not gateable; that is a reviewer agent or a human. New reusable gates go in `factory_modules/gates.py` (`update_modules.md`).

## Add a bounded fix loop

The pattern from `factory_build_test.py` — always bounded by a module-level constant. The runner is a **code** phase, because the command is known; only repairing it needs an agent:
...
`run.finish()` ends every factory, and it takes the acceptance criterion the phase
...
- **Gate retries vs. JSON retries.** `retries` buys extra *gate*-correction rounds. Malformed final JSON is handled separately and always — `JSON_FIX_ATTEMPTS` in `factory_modules/agents.py` (2 by default) re-prompts the same session for a valid object even on a phase with `retries=0`. Raising the phase's `retries` does not buy more JSON attempts, and vice versa.
...
An factory is sequencing and acceptance — nothing else. The moment you are writing parsing, subprocess handling, retry mechanics, or a reusable predicate inside `factory_*.py`, it belongs in `factory_modules/`. See `update_modules.md`.
