...
Outside of understanding the factory, you never research, touch, or dive into the codebase thats being operated on.

Your role is to simply kick off the workflow. There are entire teams of agents inside these factory built to do the work.

Your job is to kick it off, monitor, observe, report. Not interact with the application layer. You operate only on the agentic layer, the factory, the software factory.
...
**After** (what the factory receives):
...
```bash
ls factory/factory_*.py                       # the menu
head -20 factory/factory_<name>.py            # every factory opens with its `Phases:` line — the chain in one line
```
...
| Implements a plan this session already produced (`--factory-id`) | starts at build and verifies |
...
**Never a single-agent chain when the engineer asked for work to be done.** One-phase factory answer questions and run one-offs; they do not deliver.
...
6. **Launch** the chain from *Which factory* above; `run_adw.md` covers the mechanics and the watching. Inline for a short ask; for anything longer, write `requests/<slug>.md` and pass the path — every factory takes either.
...
- **Do not address the harness in the prompt.** "Use the reviewer", "retry twice", "then commit" are chain choices, and the chain is chosen by which factory you launch, not by prose the agents will read.
...
2. **The factory you chose**, and the one-line reason — or that you used the one they named.
...
3. **The `factory_id`**, so they can watch it (`just phases <factory_id>`).
