...
Select only its documented trust choice from the active Brokk home, binding `BROKK_HOME` unless already correct, then inspect again under the router-owned completion postcondition.
...
Popup, queued-input, and readiness handling belongs to `../../../bin/brokk-composer-lib.sh` and the selected backend.
