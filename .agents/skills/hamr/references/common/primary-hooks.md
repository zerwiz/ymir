...
Load this with the detected primary's tool reference before changing session startup, turn-end handling, pre-tool protection, watcher supervision, or Eindri-home integration.
...
`../../../docs/supervision-protocols/` and `../../../bin/brokk-supervision-instructions.sh` own harness-specific wake protocols.
Never substitute another harness's wait shape.
`../../../bin/brokk-busy-lib.sh` remains the semantic busy owner; a tool reference names only its source and evidence.
...
Supported primaries deny watcher-arm anti-patterns before execution, including shell `&`, truncating pipes, bundling, and broad `pkill -f brokk-watch`.
...
A primary must also account for built-in delegation that can create work outside Brokk's durable records.
