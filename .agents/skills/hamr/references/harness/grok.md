...
Tmux and Herdr now route captures through `../../../bin/brokk-composer-lib.sh`, which classifies real text on every proven content row.
`../../../docs/herdr-backend.md` owns the boundary and `../../../tests/brokk-backend-herdr.test.sh` covers it.
...
`fm_composer_strip_ghost` in `../../../bin/brokk-composer-lib.sh` drops dim or faint and truecolor below `BROKK_COMPOSER_GHOST_LUMA_MAX`, default 128.
...
Coverage is `../../../tests/brokk-composer-ghost.test.sh` and `../../../tests/brokk-backend-herdr.test.sh`.
...
Project hooks require folder trust in `~/.grok/trusted_folders.toml`, which Brokk does not edit; global `~/.grok/hooks/` is always trusted.
The spawn installs guarded global `brokk-turn-end.json` and `brokk-turn-end.sh`.
...
`.grok/hooks/brokk-primary-turnend-guard.json` invokes `../../../bin/brokk-turnend-guard-grok.sh`.
