...
The spawn scopes `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false` to every Claude worker and Eindri-home without changing global config.
CLI `--prompt-suggestions` affects print or SDK mode only and did not suppress interactive ghost text on v2.1.186.

As defense in depth, `fm_composer_strip_ghost` in `../../../bin/brokk-composer-lib.sh` removes SGR-2 runs before pending classification on styled tmux, Herdr, and Zellij readers.
...
Styled capture stays internal to the boolean detector; `brokk-peek` and model-facing captures remain plain, without escapes.
...
Project `.claude/settings.json` loads only when the exact project root is the session root; Claude does not search parents, so Brokk starts at repository root.
...
`../../../bin/brokk-subagent-pretool-check.sh` denies delegation-shaped tool names.
...
`../../../docs/subagent-guard.md` owns the contract, recommendation, `BROKK_ALLOW_SUBAGENT=1`, and applicability review.
