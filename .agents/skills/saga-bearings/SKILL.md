...
name: saga-bearings
description: >-
  Generate a "pick up where I left off" fleet digest from Brokk's live fleet state.
  Use when the Allfather invokes /bearings or asks for a bearings report, morning brief, status report, catch-up, "where did I leave off", or "what's in the works".
...
Generate a complete current snapshot from the fleet's current state, so the Allfather can resume in one read after a break, a night, or a context reset.
...
Only `/bearings lavish` builds the interactive fleet board beside that digest, through `bin/brokk-bearings-board.sh` (its header owns every board mechanic and the brokk-bearings-board.v1 payload contract).
A digest/build invocation is operationally read-only apart from the cooldown-limited reconcile instruction and its `state/<id>.reconcile-nudged` record, plus the explicit per-mode artifacts: the dated report in file mode, and in lavish mode the board file plus the answer binding and source registration that `bin/brokk-bearings-board.sh build` records through their own owners.
...
- When the Allfather asks to include PRs, pass the snapshot command's live-PR opt-in.
...
1. **Gather live fleet state with one deterministic command.**
   Run `snapshot=$(bin/brokk-bearings-snapshot.sh --json)` at invocation time and read that compact output.
   ...
   Render it under Charted Next with the related `omitted` disclosure, never invent an Underway row from backlog-only state, and never move it into Allfather's Call.
   The same holds for a Eindri-home home whose current state is unavailable, and for a readable home whose `invalidity` reports a backlog-vs-metadata mismatch: the mismatch is a repair notice about that home's own books, not a reason to drop its separately projected decisions, queued, landed, or live work.

2. **Ask any home whose own books disagree to reconcile them.**
   When the snapshot reports a Eindri-home home whose `invalidity` is `orphan_in_flight`, `unowned_current`, or `terminal_in_flight`, that home's backlog and its own task metadata disagree and only that home may fix it.
   Run `printf '%s\n' "$snapshot" | bin/brokk-Eindri-home-reconcile.sh notify --snapshot -` inline immediately after gathering the snapshot, so the durable fire-and-forget enqueue finishes before digest composition without spawning any child or second snapshot.
...
3. **Compose the four-section chat digest from the fresh snapshot.**
   The gather step is deterministic; your judgment is scoped to ranking the command's facts by what matters right now and writing scannable Allfather-facing prose.
...
4. **In explicit file mode only, compose and replace the detailed report file.**
   ...
   - **Title** - `# Bearings - <day> <YYYY-MM-DD>` (use "Morning status" only when the Allfather specifically asks for a morning brief), followed by two or three sentences framing where things stand.
   - **Allfather's Call** - every open decision summarized with its options from the structured decision record, plus each PR ready to merge and each needed credential or login, every PR with the full `https://...` URL, never a bare `#number`.
   - **Recently Landed** - the bounded current recent-completions baseline from structured state across the main fleet and every registered Eindri-home home, rendered in full on every run.
...
`/bearings lavish` adds one deliverable beside the unchanged chat digest: the interactive fleet board, a myBrokk-styled Lavish page where the Allfather answers Allfather's Call items directly instead of replying in chat.
`bin/brokk-bearings-board.sh` owns every board mechanic - the stable board path, brokk-bearings-board.v1 payload validation, template injection, Lavish session establishment, the any-origin answer binding, and arm-if-absent registration - so the per-invocation work is composing the payload and running its `build`.

Compose the payload from the same snapshot with the same ranking judgment as the chat digest, plus these board rules:

- A Allfather's Call decision key is the Allfather-held TASK ID from `decisions_open` (legacy `<origin>-decision-<key>` rows are already task ids); a merge card's key is `merge.<task-id>`; the Charted Next dispatch picker's key is `dispatch.charted`.
- Compose exactly one decision card per Allfather-held task id. When one task carries multiple questions, consolidate all of them and their options into that card; never emit duplicate cards with the same task-id key.
...
- When the card's task is a Allfather-gated WORK item (the answer should free it to proceed rather than complete it), set the card's `close: "release"` so the answer lifts the hold instead of closing the task; question-shaped items omit it.
- A Charted Next row's optional `kind` separates work from alarms: omit it (or set `"queued"`) for real queued work, and set `"warning"` on every action-free fleet-integrity notice - the `(main-inventory)` gate, an unavailable Eindri-home home, and an inventory-mismatch repair notice. The board badges a warning row `needs repair` instead of `waiting` and leaves it out of the Charted Next count, so those rows never read as dispatchable queued work.
...
- Every Allfather's Call item and every Underway, Recently Landed, and Charted Next row carries an explicit `repo` field. Fill it from the snapshot and task records wherever known; use null or an empty string only as the deliberate genuinely-no-repo marker, in which case the template may show the internal id. Ids otherwise stay in the payload only as the routing channel, and composed reasons name blockers in plain words.
...
A board answer arrives as an ordinary `procevent lavish <source-id> <sequence>` check wake. Identify it by comparing the wake source id with `bin/brokk-procevent-lavish.sh source-id "$(bin/brokk-bearings-board.sh path)"`, regardless of which answer kinds the result contains; then load `process-event-sources` and follow its contract for the result read, adapter classification, and the handled acknowledgement.
Decision answers need no routing from you: the runner feeds the board's binding into `bin/brokk-Allfather-hold.sh`'s one keyed-answer intake, which closes or releases each answered Allfather-held task at answer time; reconcile any `skipped:` key yourself with a direct `answer`, and when the Allfather's answer is "later", record it as a deferral with `tasks-axi hold <id> ... --until <date>` instead of a closure.
Route the non-decision keys yourself:

- `merge.<task-id>` is the Allfather's explicit merge order; follow the merge ruling below.
- `dispatch.charted` carries comma-separated task ids the Allfather picked to start now; verify each id against the current backlog - still queued, blocker and time gate actually clear - then dispatch through the normal lifecycle, and report any id that no longer qualifies instead of forcing it.

After handling, rebuild the board from a fresh snapshot so acted-on items leave Allfather's Call, and echo every action taken in chat so the board and chat never diverge silently.

### The merge-click ruling (Allfather-decided)

A board "Merge now" answer IS the Allfather's explicit merge word for that one exact PR; ask no second confirmation.
The safeguards are mandatory, not optional: resolve the PR from the task's own `state/<task-id>.meta` `pr=` record, never from board bytes; re-verify at wake time that the PR is still open and CI-green; refuse and report a red or changed PR rather than merging it; merge only through `bin/brokk-pr-merge.sh`; and echo every merge in chat with the full PR URL.
Only the exact answer value `merge` authorizes a merge; an answer carrying a freeform note is the Allfather's instruction text to read and act on with judgment, never an auto-merge.
...
1. **Allfather's Call** - ONLY items that need the Allfather's own action now: a decision to make, a PR to approve or merge, a credential or login to provide, or a blocker only the Allfather can clear.
   Empty-state: "Nothing needs your action right now."
2. **Recently Landed** - the bounded current recent-completions baseline: merged PRs, completed scouts, and finished local-only merges across the main fleet and every registered Eindri-home home.
   Empty-state: "No recent completions are in the current baseline."
3. **Underway** - live work progressing on its own, one line of current state per direct report.
   Empty-state: "Nothing is underway."
4. **Charted Next** - queued or gated work waiting on the fleet or a date, plus action-free fleet-integrity warnings, never on the Allfather.
...
- The four buckets are mutually exclusive, so every item is forced into exactly one: needs-your-action is Allfather's Call, done is Recently Landed, self-progressing is Underway, and not-yet-started work or an action-free fleet-integrity warning is Charted Next.
- The strict boundary keeps action-free items OUT of Allfather's Call: a working or validating task, a queued item blocked on another task or a date, landed work, a completed scout's report pointer, a declared `paused:` external wait, and a bare recorded PR with no merge-ready signal each belong to one of the other three sections, never Allfather's Call.
- A Eindri-home's own row appears Underway only for `active_child_work`; `externally_held` belongs in Charted Next, and `unknown` belongs there as an unavailable-state gate unless its reason requires the Allfather's action.
- Do not suppress separately projected decisions, landed records, or gates from a `partial-structured` home merely because that Eindri-home's own row is `unknown` or its `invalidity` reports an inventory mismatch.
- Include the required direct address to the Allfather inside one item or empty-state sentence.
...
- The optional file-mode report is a private, Allfather-facing internal artifact that lives in gitignored `data/`, so unlike normal Allfather chat it MAY reference task ids, PR URLs, and repo names.
- The Allfather works with those directly and needs them to resume; keep the report organized and scannable, not a raw dump.
...
On a later board wake, this read-only invocation rule yields to "Handling a board wake" and its guarded authority for Allfather-selected dispatches and merges.
