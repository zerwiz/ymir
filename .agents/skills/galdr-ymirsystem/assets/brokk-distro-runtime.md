...
- **File:** `state/.lock` — a bare PID, read by the Pi extensions. The primary's lock lives machine-global at `${XDG_STATE_HOME:-$HOME/.local/state}/ymir/brokk.lock`, pointed to by `state/.lock-path`; an Eindri-home keeps its own `<home>/state/.lock`.
...
- **Reclaim is OPT-IN (2026-09-25).** The extensions deploy GLOBALLY, so every `pi` session on the machine loads them and resolves one machine lock; an unrelated `pi` (measured: cwd `CodeP/learnai`) carried only `YMIR_HOME`, exactly like the primary. A session may reclaim a **missing** lock only when it was **seated here**: `bin/saga-session-start.sh` records the seated pid in `$STATE/.seated`, and `gna-pi-watch.ts`'s `isSeatedSession()` (ancestry proof against that pid) gates the reclaim — otherwise it stands down, no reclaim and no delete. An `owned` primary never reaches the gate; `releaseLockIfOwned` deletes only what it owns.
