...
install[29]{step,what,self-heals}:
  ...
  "a2a","the A2A mesh engine — a2abridge (MIT): the local directory daemon (:7777) + the bridge that gives each harness the a2a tools","bin/a2abridge-ensure.sh ensure --install fetches the pinned engine and raises its directory unit; bin/a2a-mcp.sh install wires a2abridge + engram into pi + opencode. The unit is the ENGINE's own (no Ymir template); the heart's A2A node (:8301) rides the fleet step. Repairable: bin/eir-doctor.sh a2abridge"
