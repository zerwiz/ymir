...
GALDR="$ROOT/.agents/skills/galdr"
...
done < <(find "$ROOT/config" "$ROOT/.claude" "$ROOT/.codex" "$ROOT/.cursor" "$ROOT/.agents/skills/galdr/assets/pi-boot" "$ROOT/.agents/sandbox" \
...
if [ -L "$ROOT/.agents/agents/galdr.md" ] && [ "$(readlink "$ROOT/.agents/agents/galdr.md")" = "../skills/galdr/SKILL.md" ]; then
