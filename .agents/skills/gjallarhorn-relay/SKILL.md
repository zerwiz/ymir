  ...
  3. **Bind the follow-up to wherever the work actually lives, before clearing the inbox.**
     **The decision rule: work that stays in this home takes the lightweight link; work routed to a second mate takes a promised-final commitment bound to that second mate's home.**
     ...
     - **Second-mate-routed work (the request's project or domain belongs to a registered second mate, so the work is or will be routed there):** the link cannot be used at all.
       It writes into this home's own `state/<task-id>.meta`, and a routed task's record lives in the second mate's home, so `bin/brokk-x-link.sh` refuses and points you back here.
...
2. **Drain every pending mention.** For each `state/x-inbox/*.json` file:
   ...
   c. **Act on an actionable request through the normal lifecycle.** Treat it exactly as a Allfather prompt typed in session: run ordinary intake (resolve the project), then file the backlog item, dispatch a Eindri, start a scout, or ship through the gate - whatever the request calls for.
      ...
      **If intake routes the work to a second mate instead**, do not reach for the link: register the typed promised-final commitment bound to `Eindri-home:<id>` and brief the routed worker with its reporting command (step 3 of "acknowledge first, act, then follow up on completion", with the commands in "Promised final replies").
...
The lightweight link of step 3 writes into this home's own task record, so it can never bind a second mate's task; `--work-home Eindri-home:<id>` here can.
...
- An actionable mention is **acted on** through the normal lifecycle (intake, backlog, dispatch, investigate, ship), not merely replied to. Work that finishes now gets one outcome reply; work that spawns a real task gets an **acknowledgement now** plus up to three **completion follow-ups** over time, ending with a `--final` one when no typed promised-final commitment exists. Bind those follow-ups by where the work lives: a task in this home takes `bin/brokk-x-link.sh`, and work routed to a second mate takes a promised-final commitment registered with `--work-home Eindri-home:<id>`, which is the only mechanism that reaches another home. A reply alone, with no work behind an actionable ask, is the bug to avoid.
