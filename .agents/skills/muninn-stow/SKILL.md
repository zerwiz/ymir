---
name: muninn-stow
description: Sweep the current session for uncaptured durable knowledge, file it to disk, persist the open work records this session knows are unfiled or now wrong, and curate the home's tiered, decaying startup memory before a context reset. Use when the Allfather invokes /stow (e.g. "/stow", "stow what you've learned"), before a session reset or context compaction, or periodically to keep operational memory current.
user-invocable: true
metadata:
  internal: true
---
...
This skill writes only through the existing Brokk ownership and write boundaries.
...
```markdown
- Yggdrasil pool slots share one repo, so workers must create their task branch before editing. <!--a:2026-08-03-->
- While state/.hvild exists, the away-daemon owns triage (until the afk-wake fix lands; tracked: afk-pi-wake-bypass-r1). <!--p:2026-07-20-->
...
- `pinned` - no clock is ever read for it: exempt from decay and from budget eviction, changed only through inspect-then-update when the Allfather or reality changes it, except that an explicit per-item Allfather approval may offload it under the flow below.
...
- Tier defaults are file-scoped: entries in `data/Allfather.md` and `data/Allfather-shared.md` default to `pinned` because preferences and authority boundaries do not age, and entries in `data/learnings.md` default to `aging` because operational facts must re-prove themselves.
...
- Inspect each editable file's header pointer on every pass and add or correct it; for a read-only `data/Allfather-shared.md`, leave the file byte-identical and route a missing or outdated pointer to the primary owner.
...
1. Run `bin/brokk-startup-memory-budget.sh report` before considering a write.
...
2. Read every current memory file completely: `data/Allfather.md`, `data/Allfather-shared.md`, and `data/learnings.md`.
   ...
   In a Eindri-home home, `data/Allfather-shared.md` is a read-only primary-owned input: count it, never edit it, and curate only the editable local files.
...
3. Build one whole-file retention plan before editing, ordered by likelihood of informing a future session.
   Keep in always-loaded memory only current Allfather preferences, authority and safety boundaries, recurring working style, fleet-wide or frequently relevant operating facts, and concise pointers that are expensive to rediscover.
...
7. When the total is still over budget after decay and consolidation, make aggressive reduction the default, using editable files only and in this order: archive every editable stale, superseded, or low-utility entry that is eligible for archival; consolidate tighter; run the over-budget offload sweep below and autonomously relocate every eligible non-pinned conditional entry into an already-existing allowed owner only after that owner holds it; then, only when the convergence precondition below holds, archive eligible `aging` entries oldest-reinforced-first until within budget.
   ...
   The sole exception is relocation to a JIT owner after explicit, per-item Allfather approval under the offload flow below, and that entry remains in memory until its destination is live.
8. Run `bin/brokk-startup-memory-budget.sh report` again after the complete pass.
   Finish at or below the effective budget, or open a concrete Allfather decision before ending the pass.
   A Eindri-home must explicitly report `primary-owned-shared-file-alone-exceeds-budget` when the inherited shared file alone exceeds its allowance, because local curation cannot resolve it.
   Route that constraint to the primary owner and open one concrete Allfather decision at the primary owning level that names the shortfall, with exactly these options: raise the affected home's effective budget, or explicitly approve the primary owner trimming or offloading each named shared-file entry.
   ...
   Only after every safe non-pinned archival, consolidation, offload, and eligible eviction action is exhausted may a remaining excess be attributed to pinned safety, authority, or genuine Allfather-preference entries.
   In that last-resort case, create one Allfather-held decision that names the shortfall and each relevant pinned entry, with exactly these options: raise the effective budget, or explicitly approve offloading or trimming a named pinned entry.
   Route a read-only ownership constraint to its primary owner, and make every other unresolved excess a concrete Allfather decision that names the safe action still required.
...
```markdown
## 2026-08-08 stow
- (from learnings.md, tier: perishable, reinforced: 2026-06-30) While state/.hvild exists, the away-daemon owns triage... [archived: unreinforced 39d]
```
...
Each home keeps its own archive, the archive never cascades, and truncating a grown archive is a Allfather decision, not a mechanism.
...
- Eligible by authority: only a non-pinned, dated `aging` entry that is not pending offload may be autonomously relocated to an already-existing allowed owner, while a `pinned` entry may be proposed only for explicit, per-item Allfather-approved relocation and can never be archived or autonomously offloaded for budget relief.
...
**Hard rule: the stow process never creates or writes a Brokk-repo-tracked skill.**
**Every skill stow's offload produces for a Brokk home is user-owned and local, excluded through that active home's repository-local exclude file resolved with `git -C "$home_root" rev-parse --git-path info/exclude`; contributing a lesson to the shared tracked template is a separate deliberate Allfather action, never automatic.**
...
- A user-owned local skill: a directory under `.agents/skills/<freeform-name>/` whose path is appended to the active home clone's repository-local exclude file, never to a `.gitignore`.
  Resolve `home_root` to `$BROKK_HOME` when it is set and otherwise to the Brokk code root, and anchor every destination index check, exclude-path lookup, and ignore verification to that root with `git -C "$home_root"`.
  ...
  The name is freeform with no user-vs-Brokk naming convention, the skill stays per-home and untracked, and the harness still lists and JIT-loads it because skill discovery scans the filesystem and ignores git status (verified in `docs/verification/stow-memory.md`).
...
- A project's existing committed `AGENTS.md`, for project-intrinsic knowledge useful to nearly every session of that project, through a normal Eindri ship task using `bin/brokk-ensure-agents-md.sh` and the project's registered delivery mode.
...
Forbidden destinations: any Brokk-repo-tracked skill per the hard rule; Brokk's own `AGENTS.md`, which is always-loaded for every fleet session; `docs/` alone, which is never agent-loaded on demand, though a skill body may point into docs for depth; and any committed surface for private content.
A local skill exists only in this home, so offloading an entry out of `data/Allfather-shared.md` removes it from every inheriting home's always-injected memory: the proposal must say so, and the default for shared entries is keep.
...
2. Propose pinned relocation only.
   For a pinned candidate, append a `proposed-offload` section with the same fields to the completion receipt and create or refresh one durable Allfather-held backlog item using `tasks-axi add`, `tasks-axi hold`, `tasks-axi show <id> --full`, and `tasks-axi update <id> --body-file <path>` as appropriate.
   ...
   If the Allfather never answers, nothing migrates and the held item persists, but it is never treated as budget relief.
3. Migrate an approved pinned candidate outside this pass.
   Resolve `home_root` to `$BROKK_HOME` when it is set and otherwise to the Brokk code root, then re-validate the approved local-skill destination under that root for both index absence with `git -C "$home_root"` and filesystem collision absence.
...
1. **Sweep the session for uncaptured durable knowledge.**
   Look for operational learnings, Allfather preferences expressed in passing, project-intrinsic facts, standing decisions, and undone next steps.
...
3. **Write within the existing boundaries.**
   - Allfather preferences and fleet-local operational facts belong in the destination selected by AGENTS.md after the required whole-file curation pass.
     Create `data/learnings.md` only for a genuinely new local learning with no stronger owner.
   - In a primary home, curate shared Allfather preferences only under the existing primary-authoritative shared-preference contract.
     In a Eindri-home home, route a newly discovered shared preference to the main Brokk through marked status or a document pointer instead of editing the inherited file.
   - Project-intrinsic knowledge never goes directly into a project's `AGENTS.md`.
     Route it through a normal ship task so a Eindri records it with `bin/brokk-ensure-agents-md.sh` and the project's delivery path.
   - Knowledge general to every Brokk user belongs in this repo's shared tracked material through the normal branch, no-mistakes, PR, and Allfather-merge path.
...
4. **Use inspect-then-update.**
   ...
   The only graduation moves are promotion to tracked shared material through a PR, folding a learning into the Allfather-preference destination selected by AGENTS.md, archiving a stale entry to `data/memory-archive.md`, autonomous offload of an eligible non-pinned conditional entry to an already-existing allowed owner through the reduce flow above, Allfather-approved offload of a pinned durable conditional entry to a JIT-loaded owner executed through the migration step above, or deletion of an entry that is a duplicate or already preserved through a stronger existing owner.
...
- In `data/Allfather.md` and `data/Allfather-shared.md`, every unmarked entry is simply default-pinned and remains exempt from the aging clock, legacy grace cycle, and archive-by-age; consolidation still applies, and only genuine tier deviations receive markers.
...
Report the outcome in plain Allfather-facing language with all of these facts:

- effective startup-memory budget and total estimated tokens before and after;
- one or more actions for each of `data/Allfather.md`, `data/Allfather-shared.md`, and `data/learnings.md`, using only `unchanged`, `added`, `rewritten`, `pruned`, `routed`, `archived`, or `proposed-offload`; adding or replacing a migration marker is `rewritten`, never a new action verb such as `migrated`;
...
- every unresolved exception, including a primary-owned shared-file constraint in a Eindri-home home, and every concrete Allfather decision opened for an over-budget result;
...
Run `bin/brokk-stow-cascade.sh` once the primary's own pass is done.
It enumerates each registered Eindri-home exactly once, reports that home's own budget accounting, and resolves how the sweep reaches it; its header owns the stanza fields, the bound, and the exit codes.
...
- `deferred` - a remote home with no live agent. Its memory is accounted read-only and cannot be curated from here, because there is no generic remote write path for a home's own memory files.
  ...
  Relaunching that Eindri-home is a separate decision owned by `Eindri-home-provisioning`, never something `/stow` does on its own.
...
A newly discovered shared Allfather preference still routes to the primary's `data/Allfather-shared.md` under the existing primary-authoritative contract, whichever home found it.
...
Extend the completion receipt with one entry per Eindri-home alongside the primary's own, carrying that home's budget before and after, its per-file actions, its exceptions, and whether that home swept itself or was curated from here.
Keep those entries in the same plain Allfather-facing language the rest of the receipt uses.
...
The exclusion binds the pass as a writer: proposing an offload and letting the migration step execute a Allfather-approved candidate later is not the pass storing a skill.
Every Brokk-home skill that migration produces is user-owned and local under the destinations hard rule, while an approved project-level destination is produced and shipped through that project's registered delivery path, never by stow.
Changing Brokk's tracked `.agents/skills/` or public `skills/` remains a deliberately scoped Brokk repository task through its pipeline, never a stow product.
Outside a Allfather-approved offload, generalizable knowledge still routes to shared tracked material through its pipeline and fleet-local knowledge to `data/`.
