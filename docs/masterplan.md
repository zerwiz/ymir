...
- Status: ADDED. + notes:
  ...
  - 2026-09-11 DONE — **mock purge** (Allfather directive, "no mocks in the UI"): removed the synthetic per-page stream feed (`data/feeds.ts`) and the 2.4s narration timer; the bottom Stream now shows **only live Runes over SSE**. Live paths no longer seed (`emptyState()` on boot/sign-in/realm-switch; seeds confined to the **hidden demo**). Files previews read real content via a new read-only `GET /api/file`; Forge skills list real skills via `GET /api/skills`; the PR **View diff** no longer fabricates a diff (honest unavailable/GitHub pointer). Archived the removed seed sources to `assets/mock/` (`mock.ts`, `feeds.ts`, README) for possible reuse. Law: AGENTS.md "no mocks, no examples, no placeholders in shipped runtime".
...
- 2026-09-11 — `WORKING` The well is real (W0004 + W0058): the upstream `:4602` engram server named in the supervision tree did not exist (engram ships a CLI + MCP stdio server only). Built the missing face — `bin/mimir-bridge.py` + `bin/mimir-bridge.sh` (stdlib HTTP over the engram library: `/health,/recall,/recent,/timeline,/inspect,/observe`), created `<repo>/.agents/memory/kaia.engram`, and seeded it from the 368-episode JSONL well. Wired the gate API `/api/well` to the bridge (semantic recall; local JSONL fallback) and added `/api/mimir/health`; the Well gate's Bridge tile + timeline now read live data (the hardcoded mock is gone). Registered the **engram MCP server** (`engram-mcp --db … --agent-id …`) for every harness — OpenCode (`opencode.json` `mcp.engram`), Pi (`.pi/mcp.json`, `--mcp-config`), Claude (`~/.claude.json`), Cursor (`~/.cursor/mcp.json`), Codex (`~/.codex/config.toml [mcp_servers.engram]`) — after installing the required `mcp<2` SDK (v2 renamed `FastMCP`→`MCPServer`, breaking engram). Bridge raised by `scripts/start.sh` and `bin/saga-session-start.sh`; supervision-tree command corrected; harness README §13 added and galdr→tyr re-synced. Verified: bridge up (370 episodes), semantic recall (`revenue`→1.0), MCP `initialize` handshake (engram 1.30, tools advertised), compliance 8/8, smoke 8/8.
- 2026-09-11 — `WORKING` The well shared, purified, and readable (W0004 + W0058): registered the engram MCP server for **all four OpenCode accounts** (`opencode`, `opencode-rd`, `opencode-work`, `opencode-oczer`), **Pi** (`~/.pi/agent/settings.json` + `~/.pi/agent/mcp.json` + repo `.pi/mcp.json`), Claude, Cursor, and Codex — **unscoped**, so every harness reads one shared well (per-call `agent_id` attributes writes). Purged the test/mock episodes (`smoke`/`Test observation`) from both the engram store and `episodes.jsonl` → **367 true episodes** (the no-mock law). Made Well memories **clickable** (`GET /api/well/episode?id=`, bridge `/episode`, modal with full text); fixed the bridge `/recent`/`/recall` regression. Fixed the lifecycle: `bifrost-bridge.sh`/`mimir-bridge.sh` now stop by process match when the PID file is lost and record the PID when already up, so `scripts/start.sh` and `scripts/stop.sh` raise/lower the whole system (verified: 3888/3889/4602/4603/8437). Galdr gained `assets/memory-well.md` (routed in `SKILL.md`), a Memory registry section, a corrected registry header (22→33), and harness README §13; mirrored to tyr. Verified: MCP recall 1.0 / stats 367, full stop→start cycle, compliance 8/8 (47 blocks), smoke 8/8, lint 4/4.
- 2026-09-11 — `WORKING` Chat on the real models + per-page rolling feeds + sync hardening: the Kaia chat now reads the operator's root Pi catalog (`~/.pi/agent/models.json`) for the exact provider → base URL, model id, and key (llama.cpp router `:8080`, `sk-not-key-required`); a chosen local model is tried only on its own base and never falls through to the Bifrost 401; the default is the operator's Pi default (`qwen3.6-35b-a3b@iq3_s`); `/api/chat/models` lists the real connected set (86 entries, 22 `llamacpp-*`). Verified live: `gemma-4-12b@q3_k_s`, the default 35b, and `qwen3.5-9b@q4_k_s` all answered. The bottom stream is now **per-page rolling 40** (`STREAM_WINDOW=40`), events routed module→gate with the fleet-wide feed as fallback, matching the chat's `CHAT_WINDOW=40`. Hardened the Galdr/Tyr sync by making `tyr-check/assets` a **symlink** to `galdr/assets` (a background writer had been flipping the registry red). Verified: SPA 200, compliance 8/8, smoke 8/8, lint 4/4.
- 2026-09-11 — `WORKING` Per-page feeds (header + stream): every gate now has a hardcoded, domain-appropriate rolling feed (16 pools in `src/data/feeds.ts`; extra Kaia "oracle by the well" lines for the chat page). A live `PageFeed` ticker sits in each page's header (glyph + hint, latest 3 messages), and the bottom Stream keeps each page's last 40 domain messages (fleet-wide fallback), topped up every 2.4s and seeded on first visit via `setGate`. Verified: SPA 200, compliance 8/8, smoke 8/8, lint 4/4.

- 2026-09-11 — `DECIDED` single-tenant architecture (see §6): **workspaces** (personal/work) replace realm/tenant; **domains** (company · marketing · development · life · me) become first-class; **companies** are work-space labels whose data lives in their own container (`svartalfaheim/<company>/` = the company Docker boundary, mounted by `workspace/work/`); **houses** are brands only, never a provisioning choice. Build pending. Membership: craig → owner of WayOf when user accounts return.

## 6. Architecture decision — single tenant, workspaces, company containers (2026-09-11)

**Context.** The fleet conflated three axes under "house": realm/tenant (`svartalfaheim/<owner>/`), brand (Ymir Labs, Brokk Forge, …), and domain of knowledge (`workspace/{company,marketing,development,life,me}`). Onboarding asked for a *house* to provision a *workspace*, which is brand-shaped input producing domain-shaped output.

**Decision (Allfather).** Ship **single tenant** now; defer multi-user to future containers.

```
model[6]{axis,meaning,isolation}:
  "Owner","the Allfather — one operator","—"
  "Workspace","a running scope: personal | work","THE unit; later a container"
  "Domain","company · marketing · development · life · me","folders inside a workspace"
  "Company","a work workspace's label/data (WayOf)","its own container (future)"
  "Project","belongs to a workspace; spans domains","repo + posture"
  "House","brand label (Ymir Labs, Brokk Forge, …)","display only, never isolation"
```

- A **work** workspace **shares its company's container** and sees the company's data running in it.
- **`svartalfaheim/<company>/` becomes the company's Docker boundary** (company data/image), not a person realm; `workspace/work/` mounts it.
- **Personal** workspaces later get their own container.
- **Onboarding** asks for workspace name + kind (personal/work) + domains — never a house.
- **Projects** move to `workspace/projects.yaml` with `workspace`, `company`, `domains[]`, `repo`, `posture`.
- **Membership note:** craig → owner of WayOf (deferred to when user accounts return, post single-tenant).
...
- 2026-09-12 — `ADDED` Plan 30 — Mobile (APK / PWA): `docs/plans/30-mobile.md`. Hlidskjalf is desktop-first; a responsive stylesheet is step one. Workstreams: bottom tab-bar nav + More sheet (replace the rail), workspace switcher as a sheet + safe-area insets + Android back-button, a gate-by-gate phone audit (tables→cards), a full-screen chat with sticky composer, PWA (maskable icons + offline shell), APK (adaptive icon/splash, status bar, back button, decide remote vs bundled, release signing), mobile auth/session persistence, and performance (lazy gates, SSE reconnect). Test matrix: Chrome PWA + APK + ≤360px + offline + backgrounding.
- 2026-09-12 — `WORKING` updater fleet behavior: `bin/brokk-update.sh` now updates this Brokk home **and every registered Eindri-home** (registry `data/eindri-homes.md`), fast-forward only (never forces/stashes), printing one row per target plus `reread-Brokk: yes|no` and `nudge-eindri-homes: <id>…|none` — matching the ymir-update skill. Forged the missing `bin/brokk-send.sh` (queues a one-line steer to a home's `state/.wake-queue`). Closes the skill/script mismatch.
- 2026-09-12 — `FIX` Ró preference is per-user, not tracked: `.agents/config/ro` (calm on/off) was a git-tracked file, so toggling it dirtied every user's tree and blocked the sanctioned updater. It now lives in the gitignored `state/ro` (legacy `config/ro` still read for upgrade, never written), with `YMIR_RO`/`BROKK_RO` as an env default. The tracked file is removed; `.gitignore` + `.env.example` updated.
- 2026-09-12 — `WORKING` fleet preferences: `data/fleet.md` (gitignored) holds fleet-wide per-user settings (`ro: on|off`); `bin/fleet-apply.sh` writes each into THIS home and every registered Eindri-home's gitignored `state/` (remote routes reported), and `bin/brokk-update.sh` re-applies them on every sweep. So one setting reaches the whole fleet without dirtying any tracked tree. Documented in the eindri-homes skill; smoke check updated (ro is no longer a tracked config).
